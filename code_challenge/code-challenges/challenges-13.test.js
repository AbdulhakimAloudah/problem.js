...
const longestString = (arr) => {
  ...
  return arr.findIndex((element) => element.length == maxLength);
};
...
const standardizePhoneNumbers = (arr) => {
  let myArr = [];
  arr.map((item, i) => {
    myArr[i] = "" + item.match(/\d+/g);
    myArr[i] = myArr[i].split(",").join("");
  });
  return myArr;
};
...
const sortByDay = (arr) => {
  ...
  let s = arr.map((item) => {
    daysOfWeek.map((a, i) => {
      if (item.includes(a)) myarr[i].push(item);
    });
  });
  return myarr;
};
